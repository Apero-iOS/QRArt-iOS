...
public struct TLPhotosPickerConfigure {
    ...
    public var cameraIcon = R.image.camera()
    public var videoIcon = R.image.video()
    public var placeholderIcon = R.image.insertPhotoMaterial()
...
open class TLPhotosPickerViewController: UIViewController {
    ...
    private func updateUserInterfaceStyle() {
        if #available(iOS 13.0, *) {
            ...
            let image = R.image.pop_arrow()
            let subImage = R.image.arrow()
...
extension TLPhotosPickerViewController {
    ...
    @objc open func makeUI() {
        ...
        self.popArrowImageView.image = R.image.pop_arrow()
        self.subTitleArrowImageView.image = R.image.arrow()
    ...
    private func dismiss(done: Bool, qrString: String? = nil, sourceImage: UIImage? = nil) {
        ...
        if done {
            ...
            if configure.isChooseQRMode, let qrString = qrString, let sourceImage = sourceImage {
                self.delegate?.dismissPhotoPicker(qrString: qrString, sourceImage: sourceImage)
            }
        }else {
